...
function solution(n, stations, w) {
  ...
  const newStations = []; // 새로 기지국을 설치해야 할 위치들
  ...
  for (const station of stations) {
    ...
    if (start < left) {
      ...
      const count = Math.ceil((left - start) / cover); // 전파가 통하지 않는 아파트 개수를 기지국의 전파 거리로 나누면 필요한 기지국의 개수 (소수점 나올 시 올림 해야함 전파가 통하지 않는 곳이 생기면 안되니까)
      answer += count; // 기지국의 개수 추가
      // 필요한 위치 계산
      for (let i = 0; i < count; i++) {
        newStations.push(start + i * cover);
      }
    }
